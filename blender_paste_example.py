...
if not curve or curve.type != "CURVE":
    ...
    for spline in curve.data.splines:
        for pt_info in spline.bezier_points:
            left = list(
                (mw @ pt_info.handle_left)[:2]
            )  # Convert Vector to list, take x,y only
            pt = list((mw @ pt_info.co)[:2])
            right = list((mw @ pt_info.handle_right)[:2])
...
if not segs:
    ...
    for i, (left, pt, right) in enumerate(segs):
        if i == 0:
            ...
            path_data.append(
                f"C {prev_right[0] * scale},{-prev_right[1] * scale} {left[0] * scale},{-left[1] * scale} {pt[0] * scale},{-pt[1] * scale}"
            )
    ...
    path.set("style", "stroke-linecap:round;stroke-linejoin:round")
    svg.append(path)

    print(f"Created SVG path with {len(segs)} segments")

# @local
print(f"✓ Bezier curve '{curve_name}' transferred to Inkscape!")
